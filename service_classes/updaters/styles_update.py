            ...
            new_images.append(
                {
                    ...
                    "source": "local",
                    "imageSrc": str(
                        Path.relative_to(image, project_paths.get_public_path(""))
                    ),
...
def del_removed_images_from_photos_fields(dry_run: bool = False) -> None:
    ...
    for style in styles_table:
        ...
        new_images_value = [
            img
            for img in existing_images_in_db
            if Path(img["download_path"]) in existing_images_on_disk
        ]
...
def get_scraped_images(style_name: str, search_term: str, count: int):
    ...
    search_phrases = [
        ...
        (
            f"fashion styles {search_term} aesthetic",
            count // 2 if count % 2 == 0 else count // 2 + 1,
        ),
    ]
